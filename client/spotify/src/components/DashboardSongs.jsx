...
const DashboardSongs = () => {
    ...
    if (!allSongs) {
      getAllSongs().then((data) => {
        ...
        if (data && data.length > 0) {
          // Adding an 'index' property to each song in the data array
          const songsWithIndex = data.map((song, index) => ({ ...song, index }));
  
          dispatch({
            type: actionType.SET_ALL_SONGS,
            allSongs: songsWithIndex,
          });
        }
      });
    }
  ...
  <div className="flex items-center">
    <input
      ...
      className={`w-52 md:w-64 px-4 py-2 border ${
